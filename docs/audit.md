...
5. **Offline scope:** downloads and reading position are browser-local. Pending download queues do not survive a tab/process restart; HLS segment resume is supported when restarting the same download. Offline video progress is stored locally; automatic replay to the backend on reconnect is not implemented. Online video progress continues to post to the API.
