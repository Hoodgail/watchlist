 ...
 *
 * Availability is defined in shared/providers.ts.
 */
...
import { getProviderMapping } from './api';
 ...
 */
const resolutionCache = new Map<string, { id: string; confidence: number }>();
let cachedSessionToken: string | null = null;

function getCacheKey(refId: string, provider: VideoProviderName): string {
  const token = getAccessToken();
  if (cachedSessionToken !== token) { resolutionCache.clear(); cachedSessionToken = token; }
...
export function getCachedProviderId(refId: string, provider: VideoProviderName): string | null {
  return resolutionCache.get(getCacheKey(refId, provider))?.id || null;
}

/**
 * Cache a resolved provider ID
 */
export function cacheProviderId(refId: string, provider: VideoProviderName, providerId: string, confidence = 1): void {
  if (resolutionCache.size >= 200) resolutionCache.clear();
  resolutionCache.set(getCacheKey(refId, provider), { id: providerId, confidence });
}
...
export async function resolveToProvider(
  ...
  if (cachedId) {
    const result: ResolvedMedia = {
      ...
      title: options.title,
      originalRefId: refId,
      confidence: resolutionCache.get(getCacheKey(refId, provider))?.confidence ?? 0,
      isVerified: false,
    };
  ...
  try {
    ...
    cacheProviderId(refId, provider, match.id, match.score);
...
export async function resolveWithAlternatives(
  ...
  try {
    ...
    cacheProviderId(refId, provider, bestMatch.id, bestMatch.score);
