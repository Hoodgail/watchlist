...
export async function getMediaInfo(
  ...
  const result = await getMediaInfoBase(provider, rawMediaId(provider, mediaId), mediaType);
  if (!result) {
    throw new Error(`Failed to fetch media info for ${provider}:${mediaId}`);
  }
  const scopedEpisode = (episode: VideoEpisode): VideoEpisode => ({ ...episode, id: mediaPartKey(provider, rawMediaId(provider, mediaId), episode.id) });
  return { ...result, id: createRefId(provider, mediaId),
    episodes: result.episodes?.map(scopedEpisode),
    seasons: (result as VideoMediaInfo).seasons?.map(season => ({ ...season, episodes: season.episodes.map(scopedEpisode) })),
  } as VideoMediaInfo;
}
...
export async function getEpisodeSources(
  ...
  const result = await getEpisodeSourcesBase(provider, rawMediaPartId(provider, episodeId), mediaId ? rawMediaId(provider, mediaId) : undefined);
  if (!result) {
...
export async function getEpisodeServers(
  ...
  return getEpisodeServersBase(provider, rawMediaPartId(provider, episodeId), mediaId ? rawMediaId(provider, mediaId) : undefined);
}
...
function rawMediaId(provider: string, id: string): string {
