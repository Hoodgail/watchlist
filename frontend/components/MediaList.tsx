...
const GamePlatformIcons: React.FC<{ platforms: string[] }> = ({ platforms }) => {
  const getPlatformIcon = (platform: string): React.ReactNode => {
    ...
    if (p.includes('pc') || p.includes('windows')) {
      return (
        <svg className="w-3 h-3" viewBox="0 0 24 24" fill="currentColor" title="PC">
    ...
    if (p.includes('playstation')) {
      return (
        <svg className="w-3 h-3" viewBox="0 0 24 24" fill="currentColor" title="PlayStation">
    ...
    if (p.includes('xbox')) {
      return (
        <svg className="w-3 h-3" viewBox="0 0 24 24" fill="currentColor" title="Xbox">
    ...
    if (p.includes('nintendo') || p.includes('switch')) {
      return (
        <svg className="w-3 h-3" viewBox="0 0 24 24" fill="currentColor" title="Nintendo">
    ...
    if (p.includes('ios') || p.includes('iphone') || p.includes('ipad')) {
      return (
        <svg className="w-3 h-3" viewBox="0 0 24 24" fill="currentColor" title="iOS">
    ...
    if (p.includes('android')) {
      return (
        <svg className="w-3 h-3" viewBox="0 0 24 24" fill="currentColor" title="Android">
    ...
    if (p.includes('linux')) {
      return (
        <svg className="w-3 h-3" viewBox="0 0 24 24" fill="currentColor" title="Linux">
    ...
    if (p.includes('mac')) {
      return (
        <svg className="w-3 h-3" viewBox="0 0 24 24" fill="currentColor" title="macOS">
                    ...
                    {item.type === 'GAME' ? (
                      <>
                        {item.platforms && item.platforms.length > 0 && (
                          <span className="flex items-center gap-1 text-neutral-400 m-auto">
                            <GamePlatformIcons platforms={item.platforms} />
                          </span>
                        )}
                        {item.metacritic && (
                          <span className={`text-[10px] px-1 m-auto ${item.metacritic >= 75 ? 'text-green-500' : item.metacritic >= 50 ? 'text-yellow-500' : 'text-red-500'}`}>
                            {item.metacritic}
                          </span>
                        )}
                        ...
                        {item.playtimeHours && item.playtimeHours > 0 && (
                          <span className="text-neutral-500 text-[10px] py-0.5 m-auto">
                ...
                <div className="flex gap-2 w-full justify-between">
                  ...
                  {readonly ? (
                    ...
                    >
                      {STATUS_OPTIONS
                        .filter(opt => {
                          // For games, hide WATCHING and READING
                          if (item.type === 'GAME') {
                            return opt.value !== 'WATCHING' && opt.value !== 'READING';
                          }
                          // For manga, hide WATCHING and PLAYING
                          if (item.type === 'MANGA') {
                            return opt.value !== 'WATCHING' && opt.value !== 'PLAYING';
                          }
                          // For video types (TV, MOVIE, ANIME), hide READING and PLAYING
                          return opt.value !== 'READING' && opt.value !== 'PLAYING';
                        })
                        .map(opt => (
